...
function App() {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Alt' || e.key === 'Tab' || e.key === 'Meta' || e.key === 'Control'){
        modKeyDown.current = true;
        return;
      }
      if (e.key === "Shift") {
        setCAPS(()=>true);
  ...
  return (
    <>
      ...
      {text.split(' ').filter(string=>string!='').length >= length ? <Download text={text} setText={setText}></Download> : ``}
      {text.split(' ').filter(string=>string!='').length >= length ? <p>{(text.split(' ').filter(string=>string!='').length/((Date.now() - startTime)/60_000)).toFixed(1)} WPM</p> : ``}
    </>
  )
}
